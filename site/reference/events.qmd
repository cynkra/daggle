...
| `peak_rss_kb` | integer | Peak resident set size, in KB (normalized across platforms) |
| `user_cpu_sec` | number | User-mode CPU time, in seconds |
| `sys_cpu_sec` | number | Kernel-mode CPU time, in seconds |

Resource fields come from `syscall.Rusage` after `cmd.Wait`. Linux reports `Maxrss` in KB natively; macOS reports it in bytes and daggle normalizes to KB so downstream consumers can compare cross-platform.
...
Emitted when a user attaches a note to a run via `daggle annotate` or the annotations API.

| Field | Type | Description |
|-------|------|-------------|
| `type` | string | `"run_annotated"` |
| `v` | integer | Schema version |
| `run_id` | string | Run identifier |
| `timestamp` | string | ISO 8601 timestamp |
| `author` | string | Who wrote the note (defaults to `$USER`) |
| `note` | string | Free-form note text |
