...
|-------|------|----------|---------|-------------|
| `name` | string | no | (filename without extension) | Unique DAG identifier; falls back to the YAML filename when omitted |
...
| `catchup` | enum | `"off"` (default), `"once"`, or `"all"` -- reconcile cron ticks missed while daggle was offline. Requires `schedule`. See [catchup policy](../running/triggers.qmd#catchup-policy) |
...
| `artifacts` | array | no | `[]` | Output file declarations. Each entry has `name`, `path`, `format` (optional), `versioned` (optional). See [artifacts config](#artifacts-config) |
...
| `freshness` | array | no | `[]` | Source freshness checks on input files. Each entry has `path`, `max_age`, `on_stale` (optional). See [freshness config](#freshness-config) |
